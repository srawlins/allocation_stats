...
allocations. Ruby 2.1's new `ObjectSpace.trace_allocations` method is very raw,
and does not provide useful information for anything larger than micro
profiling. The data must be aggregated!  ObjectSpace Stats collects all of the
allocation information generated by `ObjectSpace.trace_allocations`, then
provides mechanisms for filtering, grouping, and sorting allocations.

Tabular Output examples
-----------------------

It is very easy to get some simple statistics out of ObjectSpace Stats. Just
pass a block to `ObjectSpace::Stats.new` and print out a listing of all of the
object allocations that occurred within the block:

Here is `examples/allocation.rb`:
...
(For readability, I have used `alias_paths: true` here, so that the `sourcefile`
...
allocations. Through `ObjectSpace.trace_object_allocations`, one can trace the
following properties for any new object allocation:
